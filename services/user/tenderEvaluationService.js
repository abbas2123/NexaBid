...
/**
 * Get Tender Evaluation Data
 */
exports.getTenderEvaluationData = async (user, admin, queryParams, page = 1, limit = 5) => {
    const skip = (page - 1) * limit;
    const userRole = admin ? 'admin' : user?.role || 'user';
...
/**
 * Generate Tender Evaluation PDF
 */
exports.generateEvaluationPDF = async (user, admin, filters) => {
    const userRole = admin ? 'admin' : user?.role || 'user';
    const userId = user?._id;
    const { status, searchQuery, startDate, endDate } = filters;
    ...
    doc.text(`User: ${user?.name || 'Admin'}`, 400, 50, { align: 'right', width: 150 });
    ...
    tendersWithStats.forEach((t, i) => {
        ...
        doc.fillColor(statusColor).font('Helvetica-Bold').text(t.status.toUpperCase(), colX[1], y, { width: colWidths[1], ellipsis: true });
        doc.fillColor(colors.text).font('Helvetica');
        doc.text(t.awardedTo?.name || '-', colX[2], y, { width: colWidths[2], ellipsis: true, lineBreak: false });
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
        doc.switchToPage(i);
        doc.fontSize(8).fillColor(colors.gray).text(`Page ${i + 1} of ${range.count}`, 0, doc.page.height - 30, { align: 'center' });
    }
