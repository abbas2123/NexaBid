...
exports.uploadPublisherAgreement = async ({ tenderId, publisherId, file }) => {
  ...
  if (!winnerBid) {
    throw new Error(ERROR_MESSAGES.WINNER_NOT_FOUND);
  }

  const { vendorId } = winnerBid;
...
exports.approveAgreement = async (agreementId) => {
  ...
  if (!agreement) {
    throw new Error(ERROR_MESSAGES.AGREEMENT_NOT_FOUND);
  }
...
exports.issueWorkOrder = async (publisherId, tenderId, body, file) => {
  if (!file) throw new Error(ERROR_MESSAGES.WORK_ORDER_PDF_REQUIRED);
  if (!body.description) throw new Error(ERROR_MESSAGES.WORK_ORDER_DESCRIPTION_REQUIRED);
...
exports.getWorkOrderFilePath = async (publisherId, fileId) => {
  ...
  if (!workOrder.fileUrl) {
    throw new Error(ERROR_MESSAGES.FILE_URL_MISSING);
  }
  return path.join(process.cwd(), workOrder.fileUrl.replace('/uploads', 'uploads'));
};
